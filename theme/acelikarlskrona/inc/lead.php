 ...
 * or photo. They call ac_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional AC El I Karlskrona AB.
 *
 ...
 * @return string
 */
function ac_sample_note() {
	return ac_lead( 'ar_riktig' ) ? '' : ' <span class="ac-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ac_lead_img().
 */
function ac_lead_bild( $i ) {
	$bilder = (array) ac_lead( 'bilder', array() );
 ...
 * @return string
 */
function ac_ring_label() {
	$namn = ac_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ac_lead( 'telefon' ) ) : 'Ring ' . ac_lead( 'telefon' );
}
