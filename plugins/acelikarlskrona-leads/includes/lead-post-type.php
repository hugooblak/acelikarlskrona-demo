...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'acelikarlskrona-leads' ),
					'singular_name' => __( 'Lead', 'acelikarlskrona-leads' ),
					'edit_item'     => __( 'Förfrågan', 'acelikarlskrona-leads' ),
					'search_items'  => __( 'Search leads', 'acelikarlskrona-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'acelikarlskrona-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'acelikarlskrona-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'acelikarlskrona-leads' ),
			'npl_phone'    => __( 'Phone', 'acelikarlskrona-leads' ),
			'npl_service'  => __( 'Needs', 'acelikarlskrona-leads' ),
			'npl_timeline' => __( 'When', 'acelikarlskrona-leads' ),
			'npl_zip'      => __( 'ZIP', 'acelikarlskrona-leads' ),
			'npl_source'   => __( 'Source', 'acelikarlskrona-leads' ),
			'npl_status'   => __( 'Status', 'acelikarlskrona-leads' ),
			'date'         => __( 'Received', 'acelikarlskrona-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'acelikarlskrona-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'acelikarlskrona-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'acelikarlskrona-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'acelikarlskrona-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'acelikarlskrona-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'acelikarlskrona-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'acelikarlskrona-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'acelikarlskrona-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'acelikarlskrona-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'acelikarlskrona-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'acelikarlskrona-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'acelikarlskrona-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
